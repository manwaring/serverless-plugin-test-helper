...
/*
 * @deprecated This method has been deprecated and will be removed in the next major release, use `new SNSEvent()` instead
 */
export function snsEvent(override: NestedPartial<SNSEvent> = {}): SNSEvent {
  // console.warn(
  //   "This method has been deprecated and will be removed in the next major release, use `new SNSEvent()` instead"
  // );
...
export class SnsEvent {
  ...
  constructor(override: NestedPartial<SNSEvent> = {}) {
    const Records = override.Records
      ? all([defaultRecords, override.Records], { arrayMerge: combineMerge })
      : defaultRecords;
    this.Records = Records as SNSEventRecord[];
  }
}
